...
export const I18N_COPY = {
  zh: {
    language: "语言",
    title: "CJK 字体转换器",
    description: "生成固定 UI 字号与自定义阅读字号的 .cpfontpkg 字体家族包；旧版 Bin 与 XBF2 位于旧格式工具。",
    fontLibrary: "字体库",
    makeCpfont: "新格式工具",
    legacyTools: "旧格式工具",
    outputFormatCpfont: ".cpfontpkg 字体家族包",
    cpfontSizes: "UI 字号固定为 8 / 10 / 12 pt",
    cpfontPreset: "生成预设",
    cpfontPresetFamily: "阅读字体包",
    cpfontPresetUi: "UI 字体包",
    cpfontPresetFamilyHint: "生成固定 UI 字号与自定义阅读字号。",
    cpfontPresetUiHint: "仅生成 UI 使用的 8 / 10 / 12 pt。",
    readerSizes: "阅读字号 (pt)",
    readerSizesHint: "输入以逗号分隔的任意整数，默认 14, 16, 18, 22",
    readerSizesInvalid: "请输入 1–255 之间、以逗号分隔的阅读字号",
    cpfontIntervals: "字符范围：Latin Extended + CJK",
    familyName: "字体家族 ID",
    forceAutohint: "强制 FreeType 自动 Hinting",
    previewApproximate: "上传预览使用浏览器源字体近似渲染，并非最终 .cpfont 点阵。",
    fileLimit: "最大上传 20 MiB；支持 .ttf / .otf",
    ...
    outputFormat: "导出格式",
    outputFormatLegacy: "Legacy Bin",
    outputFormatXbf2: "XBF2 实验格式",
    fontSizePx: "字号 (px)",
    fontWeight: "字重",
  ...
  ja: {
    ...
    description: "固定 UI サイズと任意の読書サイズを含む .cpfontpkg ファミリーパッケージを生成します。旧 Bin と XBF2 は旧形式ツールにあります。",
    fontLibrary: "フォントライブラリ",
    makeCpfont: "新形式ツール",
    legacyTools: "旧形式ツール",
    outputFormatCpfont: ".cpfontpkg フォントファミリーパッケージ",
    cpfontSizes: "UI サイズは 8 / 10 / 12 pt 固定",
    cpfontPreset: "生成プリセット",
    cpfontPresetFamily: "読書フォントパッケージ",
    cpfontPresetUi: "UI フォントパッケージ",
    cpfontPresetFamilyHint: "固定 UI サイズと任意の読書サイズを生成します。",
    cpfontPresetUiHint: "UI 用の 8 / 10 / 12 pt のみを生成します。",
    readerSizes: "読書サイズ (pt)",
    readerSizesHint: "カンマ区切りの整数を入力。初期値: 14, 16, 18, 22",
    readerSizesInvalid: "1～255 の読書サイズをカンマ区切りで入力してください",
    cpfontIntervals: "文字範囲: Latin Extended + CJK",
    familyName: "フォントファミリー ID",
    forceAutohint: "FreeType 自動ヒンティングを強制",
    previewApproximate: "アップロードプレビューはブラウザーによる元フォントの近似で、最終 .cpfont ビットマップではありません。",
    fileLimit: "最大 20 MiB、.ttf / .otf 対応",
  ...
  en: {
    ...
    description: "Generate a .cpfontpkg family with fixed UI sizes and custom reader sizes. Legacy Bin and XBF2 are available under Legacy Tools.",
    fontLibrary: "Font Library",
    makeCpfont: "New Format Tools",
    legacyTools: "Legacy Tools",
    outputFormatCpfont: ".cpfontpkg font family package",
    cpfontSizes: "UI sizes are fixed at 8 / 10 / 12 pt",
    cpfontPreset: "Generation preset",
    cpfontPresetFamily: "Reader font package",
    cpfontPresetUi: "UI font package",
    cpfontPresetFamilyHint: "Generate fixed UI sizes plus custom reader sizes.",
    cpfontPresetUiHint: "Generate only the 8 / 10 / 12 pt UI sizes.",
    readerSizes: "Reader sizes (pt)",
    readerSizesHint: "Enter comma-separated integers; defaults to 14, 16, 18, 22",
    readerSizesInvalid: "Enter comma-separated reader sizes from 1 to 255",
    cpfontIntervals: "Character coverage: Latin Extended + CJK",
    familyName: "Font family ID",
    forceAutohint: "Force FreeType auto-hinting",
    previewApproximate: "The upload preview is an approximate browser rendering of the source font, not the final .cpfont bitmap.",
    fileLimit: "20 MiB upload limit; supports .ttf / .otf",
    ...
    outputFormatLegacy: "Legacy Bin",
    outputFormatXbf2: "Experimental XBF2",
    fontSizePx: "Size (px)",
    fontWeight: "Weight",
...
export async function runWebFlow(
  ...
  const created = await apiRequest("/api/jobs", {
    ...
    body: JSON.stringify({
      ...
      force_autohint: input.forceAutohint,
      reader_sizes: input.readerSizes,
      package_role: input.packageRole,
